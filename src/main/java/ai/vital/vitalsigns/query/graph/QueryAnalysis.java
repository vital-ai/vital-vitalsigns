...
//		if(rootContainers.size() < 1) ex("No criteria containers found in an ARC");
		...
		if(rootContainers.size() == 0) {
			...
			if(topContainer.getType() != QueryContainerType.and) {
			    
//			    ex("Top criteria container must be of type AND");
			    VitalGraphCriteriaContainer newContainer = new VitalGraphCriteriaContainer(QueryContainerType.and);
			    newContainer.add(topContainer);
...
//				List<Class<? extends GraphObject>> types = new ArrayList<Class<? extends GraphObject>>();
//				
//				if(c == null) {
//				    Object value = tc.getValue();
//				    if(!(value instanceof Collection)) throw new RuntimeException(VitalGraphQueryTypeCriterion.class.getSimpleName() + " value must be a collection ");
//				    Collection x = (Collection) value;
//				    for(Object y : x) {
//				        if(!(y instanceof Class)) {
//				            throw new RuntimeException(VitalGraphQueryTypeCriterion.class.getSimpleName() + " value must be a collection of Class<? extends Grap");
//				        }
//				    }
//				} else {
//				    types.add(c);
//				}
				...
				if(VitalGraphQueryPropertyCriterion.URI.equals(pURI)) {
					...
					if(pm == null) {
					    if(VitalSigns.get().getConfig().externalProperties) {
					        externalProperty = true;
					    } else {
					        ex("Property with URI not found: " + pURI + " (and external properties disabled)");
					    }
					} else {
				...
				*/
				...
				if(GraphElement.Connector == pc.getSymbol()) {
					...
					if(arc.getArc().connector == Connector.EDGE) {
						if(!externalProperty && ! isEdgeClass) {
							ex("Only edge property constraints may be used in edge arc");
						}
					} else {
						if(!externalProperty && !isHyperEdgeClass) {
					...
					if(arc.getArc().connector == Connector.EDGE) {
						...
						if(!externalProperty && !isNodeClass) ex("Only node constraints may be used as endpoint constraints in edge arc");
	...
	private static void analyzeContainer(WrappedContainer wrapped, VitalGraphCriteriaContainer cc) {
		...
		for( VitalGraphQueryElement el : cc ) {
			...
			if(el instanceof VitalGraphQueryTypeCriterion) {
				...
				} else if(VITAL_HyperNode.class.isAssignableFrom(c)) {
				    wrapped.endpointCriteria++;
				} else if(VITAL_HyperEdge.class.isAssignableFrom(c)) {
				    wrapped.connectorCriteria++;
				} else {
					ex("only node/edge/hyperedge/hypernode constraints allowed, invalid type: " + c);
				}
				...
				GraphElement symbol = ((VitalGraphQueryPropertyCriterion) el).getSymbol();
                ...
                if(VitalGraphQueryPropertyCriterion.URI.equals(pURI)) {
                    ...
                    if(symbol == null) ex("Symbol not set for URI property constraint");
                    
                    if(symbol == GraphElement.Source || symbol == GraphElement.Destination) {
                    ...
                    for( ClassMetadata domain : pm.getDomains() ) {
                        if(VITAL_Node.class.isAssignableFrom(domain.getClazz())
                                || VITAL_HyperNode.class.isAssignableFrom(domain.getClass())) {
                            isNodeClass = true;
                        } else if(VITAL_Edge.class.isAssignableFrom(domain.getClazz())
                                || VITAL_HyperEdge.class.isAssignableFrom(domain.getClass())) {
                            isEdgeClass = true;
                        }
                        if(s.length() > 0) {
                            s += ", ";
                    ...
                    if( symbol == null) {
                        ...
                        if(symbol == GraphElement.Source || symbol == GraphElement.Destination) {
                            ...
                            if(!isNodeClass) ex("Not a node/hypernode property: " + pURI);
                            
                        } else if(symbol == GraphElement.Connector) {
                            
                            if(!isEdgeClass) ex("Not an edge/hyperedge property: " + pURI);
